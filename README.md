...
V1.6 系列（龙头识别、机会指数、买卖条件引擎）已完成。

V1.7.1 新增本地 SQLite 持仓表，支持真实/模拟持仓录入、编辑和关闭，自动补全股票和板块信息，可选择保存建仓时的 V1.6 判断快照，并支持 CSV / Markdown 导出。
...
| 版本 | 能力 | 入口 |
|------|------|------|
| V1.7.1 | 持仓录入与持仓表：真实/模拟持仓、成本仓位、买入理由、V1.6快照、CSV/MD导出 | `src/portfolio/position_service.py` |
| V1.7.2 | 每日持仓体检：7维评分+原始逻辑检验+条件化动作建议+SQLite历史 | `src/portfolio/position_diagnosis_service.py` |
| V1.7.3 | 组合风险控制：仓位集中度/板块集中/相关性/回撤/连续亏损/市场适配 | `src/portfolio/portfolio_risk_service.py` |
...
1. **暂不做每日持仓体检** — `leader_structure` 固定为 `pending_v1.6.1`（V1.6.1 起已实现龙头识别，此描述待更新）
2. **不输出具体股票买卖建议** — 所有 `suggested_action` 仅为"观察/关注/等待"级别
3. **不自动实盘交易** — 系统不接任何交易接口
4. **暂不做组合风险控制** — 仓位合计仅做数据完整性提醒
5. **部分指标为近似计算**：
...
6. **数据不足时优雅降级** — 输出 `unknown` 或空列表，不崩溃
